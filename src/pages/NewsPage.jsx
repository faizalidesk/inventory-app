...
import {
  ...
  Share2, Bookmark, Filter, CheckCircle2, ChevronLeft
} from "lucide-react";
      ...
      <main className="site-wrap py-8 px-4 max-w-6xl mx-auto">
        {activeCategory === "all" && !searchQuery && currentPage === 1 && heroArticle && (
          <section className="mb-12">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
              <div className="lg:col-span-7 bg-card/60 backdrop-blur-xs border border-border/80 rounded-2xl p-6 sm:p-8 flex flex-col justify-between hover:border-primary/50 transition-all group">
                <div>
                  ...
                  <Link to={`/news/${heroArticle.id || heroArticle.slug}`} className="no-underline text-foreground">
                    <h3 className="text-2xl sm:text-3xl font-black tracking-tight leading-tight group-hover:text-primary transition-colors mb-4">
                      {heroArticle.title}
                    </h3>
                  </Link>
                ...
                <div className="flex items-center justify-between pt-4 border-t border-border/60 text-xs text-muted-foreground">
                  ...
                  <Link to={`/news/${heroArticle.id || heroArticle.slug}`} className="font-bold text-primary flex items-center gap-1 hover:gap-2 transition-all no-underline">
              ...
              <div className="lg:col-span-5 flex flex-col gap-4">
                {sideArticles.map((item, idx) => (
                  <Link key={item.id || idx} to={`/news/${item.id || item.slug}`} className="bg-card/40 backdrop-blur-xs border border-border/70 rounded-xl p-4 sm:p-5 hover:border-primary/50 transition-all group flex flex-col justify-between no-underline">
                    <div>
                      ...
                      <h4 className="font-bold text-sm sm:text-base text-foreground group-hover:text-primary transition-colors leading-snug line-clamp-2 mb-2">
                        {item.title}
                      </h4>
          ...
          {paginatedArticles.length === 0 ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {paginatedArticles.map((article, idx) => (
                <article key={article.id || idx} className="bg-card/50 backdrop-blur-xs border border-border/80 rounded-2xl p-5 hover:border-primary/50 transition-all flex flex-col justify-between group">
                  <div>
                    ...
                    <Link to={`/news/${article.id || article.slug}`} className="no-underline text-foreground">
                      <h3 className="font-bold text-base text-foreground group-hover:text-primary transition-colors leading-snug mb-2.5 line-clamp-2">
                        {article.title}
                      </h3>
                    </Link>
                    <p className="text-xs text-muted-foreground leading-relaxed line-clamp-3 mb-4">{article.summary}</p>
                  </div>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-center gap-2 mb-16">
            <button onClick={() => setCurrentPage(p => Math.max(1, p - 1))} disabled={currentPage === 1} className="p-2 rounded-xl border border-border/80 bg-card/60 text-foreground disabled:opacity-30 disabled:cursor-not-allowed hover:border-primary transition-all">
              <ChevronLeft className="w-4 h-4" />
            </button>
            <div className="flex items-center gap-1 font-mono text-xs">
              {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                <button key={page} onClick={() => setCurrentPage(page)} className={`w-8 h-8 rounded-xl font-bold transition-all ${currentPage === page ? "bg-primary text-primary-foreground shadow-xs" : "bg-card/40 border border-border/80 text-muted-foreground hover:text-foreground hover:border-primary/50"}`}>
                  {page}
                </button>
              ))}
            </div>
            <button onClick={() => setCurrentPage(p => Math.min(totalPages, p + 1))} disabled={currentPage === totalPages} className="p-2 rounded-xl border border-border/80 bg-card/60 text-foreground disabled:opacity-30 disabled:cursor-not-allowed hover:border-primary transition-all">
              <ChevronRight className="w-4 h-4" />
            </button>
          </div>
        )}

        <section className="bg-gradient-to-r from-primary/10 via-card to-primary/5 border border-primary/20 rounded-3xl p-6 sm:p-10 mb-8 text-center">
          <div className="max-w-xl mx-auto">
            <h3 className="text-xl sm:text-2xl font-black tracking-tight mb-2">Dapatkan Rangkuman Warta Terpilih Tiap Minggu</h3>
            <p className="text-xs sm:text-sm text-muted-foreground mb-6">Kurasi berita terkini mengenai kemajuan teknologi, sains, dan inovasi web langsung ke kotak masuk Anda.</p>
            {isSubscribed ? (
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-emerald-500/10 border border-emerald-500/30 text-emerald-400 rounded-xl text-xs font-mono font-bold">
                <CheckCircle2 className="w-4 h-4" /> Anda telah terdaftar dalam buletin warta Desktopalie!
              </div>
            ) : (
              <form onSubmit={handleSubscribe} className="flex flex-col sm:flex-row gap-2 max-w-md mx-auto">
                <input type="email" placeholder="Masukkan alamat email Anda..." value={subscribedEmail} onChange={(e) => setSubscribedEmail(e.target.value)} className="flex-1 bg-background/90 border border-border rounded-xl px-4 py-2.5 text-xs text-foreground placeholder:text-muted-foreground focus:outline-none focus:border-primary font-mono" required />
                <button type="submit" className="px-5 py-2.5 bg-primary text-primary-foreground font-bold text-xs rounded-xl hover:bg-primary/90 transition-all font-mono shrink-0 shadow-sm">
                  Langganan
                </button>
              </form>
            )}
